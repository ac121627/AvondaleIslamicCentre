...
namespace AvondaleIslamicCentre.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        private readonly SignInManager<AICUser> _signInManager;
        private readonly UserManager<AICUser> _userManager;
        private readonly IUserStore<AICUser> _userStore;
        private readonly IUserEmailStore<AICUser> _emailStore;
        private readonly ILogger<RegisterModel> _logger;
        private readonly IEmailSender _emailSender;

        public RegisterModel(
            UserManager<AICUser> userManager,
            IUserStore<AICUser> userStore,
            SignInManager<AICUser> signInManager,
        ...
        private AICUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<AICUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(AICUser)}'. " +
                    $"Ensure that '{nameof(AICUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<AICUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<AICUser>)_userStore;
        }
    }
}
